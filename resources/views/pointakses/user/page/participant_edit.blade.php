...
<div class="container">
    ...
    <form action="{{ route('user.participant.update', ['event_id' => $event_id, 'participant_id' => $participant->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <a href="{{ route('user.participant.index', ['event_id' => $event_id]) }}" class="btn btn-secondary">Kembali</a>
    </form>
</div>
@endsection
